...
def search_coles(query):
    try:
        ...
        response = session.get(COLES_BASE_URL, headers=COMMON_HEADERS)
    ...
    search_url = (
        f"{COLES_BASE_URL}/_next/data/{build_id}/en/search/products.json?q={query}"
    )
    try:
        response = session.get(search_url, headers=COMMON_HEADERS)
        response.raise_for_status()
        data = response.json()
        raw_results = (
            data.get("pageProps", {}).get("searchResults", {}).get("results", [])
        )
            ...
            products.append(
                {
                    ...
                    "norm_unit": norm_unit,
                    "on_sale": (pricing.get("promotionType") in ["SPECIAL", "DOWNDOWN"])
                    or (pricing.get("was", 0) > 0),
                }
            )
        return products
    except:
...
def search_woolworths(query):
    ...
    try:
        session = requests.Session()
        session.get(
            "https://www.woolworths.com.au/", headers={"User-Agent": USER_AGENT}
        )
        session.cookies.set("WOL-StoreId", "1265", domain=".woolworths.com.au")

        response = session.post(
            WOOLIES_SEARCH_URL, headers=COMMON_HEADERS, json=payload
        )
        response.raise_for_status()
        data = response.json()
        ...
        for r in raw_results:
            ...
            is_on_sale = (
                r.get("IsSpecial", False)
                or r.get("IsBundle", False)
                or "Low Price" in (r.get("LabelHtml", "") or "")
            )
...
def search_aldi(query):
    ...
    try:
        ...
        matches = re.findall(
            r'"name":"([^"]+)","brandName":"[^"]*".*?"price":{"amount":\d+,"amountRelevant":\d+,"amountRelevantDisplay":"([^"]+)".*?"comparisonDisplay":"([^"]+)"',
            content,
        )
...
def main():
    ...
    print(
        f"{'Store':<12} | {'Name':<50} | {'Price':<8} | {'Unit Price':<15} | {'Sale'}"
    )
    print("-" * 100)
    for p in rankable:
        ...
        price_val = (
            ...
            or (isinstance(p["price"], str) and p["price"] != "N/A")
            else "N/A"
        )
        print(
            f"{p['store']:<12} | {p['name']:<50} | {price_val:<8} | {p['unit_price_str']:<15} | {sale_star}"
        )
